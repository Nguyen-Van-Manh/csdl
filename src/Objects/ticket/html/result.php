...
$cart = $_SESSION["username"];
$userID = intval($_SESSION["userID"]);
if (isset($_POST["add"])) {
    if (isset($_SESSION[$cart])) {
        $item_id = array_column($_SESSION[$cart], "id");
        $count = count($_SESSION[$cart]);
        $_SESSION[$cart][$count] = intval($_GET["id"]);
    } else {
        $_SESSION[$cart][0] = intval($_GET["id"]);
    }
    $size = count($_SESSION[$cart]);
    $index = intval($_SESSION[$cart][$size-1]);
    $quantity = intval($_SESSION["quantity"]);
    $query2 = "SELECT giaVe FROM chuyenbay WHERE idChuyenBay = '$index'";
    $result2 = $connect->query($query2);
    $row2 = $result2->fetch_array(MYSQLI_ASSOC);
    $price = doubleval($row2["giaVe"]);
    $total = $price * $quantity;
    $query = "insert into datve(idChuyenBay, userID, quantity, total, status) VALUES ('$index', '$userID', '$quantity', '$total', 'NO')";
    if ($connect->query($query) == true) {
        echo '<script>window.alert("Thêm vé thành công! Đang chờ xác nhận từ đơn vị đối tác.")</script>';
    } else {
        echo "Error.";
        exit;
    }
...
<div class="container" style="margin-top: 250px; margin-bottom: 100px;">
    ...
    <div class="table-responsive" style="margin-top: 30px;">
        <table class="table table-bordered">
            ...
            <?php
            if (isset($_SESSION["date1"]) && isset($_SESSION["idLoTrinh"])) {
                $date = date("Y-m-d", strtotime($_SESSION["date1"]));
                $idLoTrinh = intval($_SESSION["idLoTrinh"]);
                $hangGhe = $_SESSION["hangGhe"];
                $sql = "SELECT *FROM chuyenbay WHERE idLoTrinh = '$idLoTrinh' AND ngayBay = '$date'";
                $result = mysqli_query($connect, $sql);
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_array($result)) {
                        ?>
                        <tr>
                            <td><?php echo $row["hangBay"] ?></td>
                            <td><?php echo $row["tenChuyenBay"] ?></td>
                            <td><?php echo $_SESSION["start"] ?></td>
                            <td><?php echo $_SESSION["end"] ?></td>
                            <td><?php echo date('d/m/Y', strtotime($row["ngayBay"])); ?></td>
                            <td><?php echo $row["gioBay"] ?></td>
                            <td><?php
                                switch (intval($row["ticketID"])) {
                                    case 1:
                                        echo "Phổ thông";
                                        break;
                                    case 2:
                            ...
                            <td>
                                <form action="result.php?action=add&id=<?php echo $row["idChuyenBay"]; ?>"
                                      method="post">
                                    <input type="submit" class="btn-success" name="add" value="Add"
                                           style="border: none;" id="add_to_cart"/>
                                </form>
                            </td>
                        </tr>
                        <?php
